# Metaframeworks - Tanstack Start: Casas Rurales
...
- `/houses`: utiliza **SSR** mediante un route loader, ya que el contenido depende de los `search params` utilizados por el buscador. Los datos del listado se mantienen en caché durante 10 segundos mediante `staleTime`.
- `/houses/$id`: utiliza **SSG mediante prerendering**. El número de casas es reducido y sus rutas son conocidas durante el build, por lo que se generan las páginas estáticas previamente.
- Buscador: es un **Client Component** porque necesita gestionar la interacción del usuario y actualizar los `search params`.
- Botón de reservar: es un **Client Component** para gestionar el estado de la reserva y ejecutar la acción de reserva.
...
`PUBLIC_BASE_PICTURES_URL`: URL base utilizada para cargar las imágenes de las casas.
